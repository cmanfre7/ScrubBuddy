...
export function OverviewTab() {
  ...
  const avgPercentile = shelfScores.length > 0
    ? Math.round(
        shelfScores.reduce((sum, s) => sum + (s.percentile || 0), 0) / shelfScores.length
      )
    : 0
            ...
            <table className="w-full">
              ...
              <tbody>
                {shelfScores.map((shelf) => {
                  const nationalAvg = 70 // Approximate national average
                  const delta = shelf.score - nationalAvg
                        ...
                        <div className="flex items-center gap-2">
                          <div className="w-20 h-1.5 bg-slate-700/50 rounded-full overflow-hidden">
                            <div
                              className={cn(
                                'h-full rounded-full',
                                delta > 0 ? 'bg-green-500' : delta < 0 ? 'bg-red-500' : 'bg-blue-500'
                              )}
                              style={{
                                width: `${Math.min(Math.abs((delta / 20) * 100), 100)}%`,
                              }}
                            />
                          </div>
                          <span
                            className={cn(
                              'text-xs font-medium',
                              delta > 0 ? 'text-green-400' : delta < 0 ? 'text-red-400' : 'text-blue-400'
                            )}
                          >
                            {delta > 0 ? '+' : ''}
                            {delta}
